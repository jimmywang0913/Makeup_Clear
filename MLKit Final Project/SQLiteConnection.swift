...
class SQLiteConnect {
    ...
    func openDatabase(_ path :String) -> OpaquePointer? {
        ...
        if sqlite3_open_v2(path, &connectdb, SQLITE_OPEN_READWRITE, nil) == SQLITE_OK {
    ...
    func fetch(
        ...
        dbStatus = sqlite3_prepare_v2(self.db, sql.cString(using: String.Encoding.utf8), -1, &statement, nil)
        print (dbStatus)
        if (statement == nil){
            return nil
        }
